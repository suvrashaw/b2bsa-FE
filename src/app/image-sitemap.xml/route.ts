...
export const GET = () => {
  const blogEntries = SHARED_BLOG_POSTS.filter((p) => p.body && p.image).map(
    (post) =>
      imageEntry(
  ...
  const caseStudyEntries = CASE_STUDY_DETAILS.filter((s) => s.image).map(
    (study) =>
      imageEntry(
  ...
  const eventEntries = TRADE_SHOW_CALENDAR_EVENTS.filter((e) => e.image).map(
    (event) =>
      imageEntry(
