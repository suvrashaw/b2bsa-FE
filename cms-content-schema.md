...
- `event-lead-generation` re-exports its parent hub's (`sales-qualified-lead-generation`) case-studies, contact, FAQ, process, services, and why content wholesale — only its hero (partial override), client logos, intro, page metadata, and unique `pipeline.json` are genuinely its own. **Not yet resolved — scheduled for Batch 4.** (Correction: case-studies is not actually part of this wholesale-borrowing list — `event-lead-generation` already has its own `case-studies.json` wrapper, only merged with the same shared `GLOBAL_CASE_STUDIES` pool every other page uses, same as everyone else. The real wholesale borrows are contact/FAQ/process/services/why — 5, not 6.)
...
- The top-level `/services` hub borrows `HUB_WHY` from `tradeshow-booth-solutions/why-spotlight.json` and `HUB_CONTACT_CTA` from `global-event-solutions/contact.json`. **Not yet resolved — scheduled for Batch 5.**
